"use strict";
const CACHE_NAME="apprenticeship-plus-bench-navigation-order-recovery-v1";
const APP_FILES=[
 "./","./index.html","./app.js",
...
self.addEventListener("fetch",event=>{
 ...
 if(url.pathname.endsWith("/courses/bench/app.js")){
  event.respondWith(fetch(event.request,{cache:"no-store"}).then(r=>r.text()).then(source=>{
   const loader=`
;(function(){
 ['./professional-navigation.css?v=recovery-1','./page-refinement.css?v=recovery-1'].forEach(function(href){
  const link=document.createElement('link');link.rel='stylesheet';link.href=href;document.head.appendChild(link);
 });
 ['./professional-navigation.js?v=recovery-1','./page-refinement.js?v=recovery-1'].forEach(function(src){
 ...
 const fresh=/\/courses\/bench\/(?:$|index\.html$|professional-navigation\.(?:js|css)$|page-refinement\.(?:js|css)$|workbench-data\.js$|workbench\.js$|assignment-pdf\.js$|manifest\.json$)/.test(url.pathname);
